    ...
    return (
        <div className="page-gap">
            <section className="grid gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)]">
                <article className="rounded-[40px] border border-[var(--border)] bg-[var(--panel)] px-6 py-7 shadow-[var(--shadow-soft)] sm:px-8">
                    <div className="text-xs font-semibold uppercase tracking-[0.28em] text-[var(--foreground-subtle)]">Apie Mono Studio</div>
                    ...
                    <p className="mt-5 max-w-3xl text-sm leading-7 text-[var(--foreground-muted)]">
                        Mono Studio renkasi švarius siluetus, neutralius tonus ir patogius audinius, prie kurių lengva grįžti kiekvieną dieną.
                    </p>
                    <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                        <Button size="lg" onClick={onBrowseProducts}>
                            Peržiūrėti kolekciją
                        </Button>
                        <Button as="a" href={onOpenSwagger} target="_blank" rel="noreferrer" variant="secondary" size="lg">
            ...
            <section className="rounded-[36px] border border-[var(--border)] bg-[var(--panel)] p-6 shadow-[var(--shadow-soft)]">
                <div className="text-xs font-semibold uppercase tracking-[0.26em] text-[var(--foreground-subtle)]">Svarbiausia</div>
            ...
            <section className="grid gap-4 xl:grid-cols-2">
                {PROJECT_FEATURES.map((feature) => (
                    ...
                    >
                        <div className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--foreground-subtle)]">Mono Studio</div>
                        <h2 className="mt-4 text-2xl font-semibold tracking-tight text-[var(--foreground-strong)]">{feature.title}</h2>
            ...
            <section className="rounded-[36px] border border-[var(--border)] bg-[var(--panel)] p-6 shadow-[var(--shadow-soft)]">
                <div className="flex flex-col gap-3 border-b border-[var(--border)] pb-5 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <div className="text-xs font-semibold uppercase tracking-[0.26em] text-[var(--foreground-subtle)]">Mūsų požiūris</div>
                    ...
                    <Button as="a" href={onOpenSwagger} target="_blank" rel="noreferrer" variant="ghost">
                        Daugiau informacijos
                    </Button>
                </div>
                ...
                <div className="mt-6 grid gap-4 xl:grid-cols-5">
                    {ARCHITECTURE_PILLARS.map((pillar) => (
                        <article
                            key={pillar.title}
                            className="rounded-[26px] border border-[var(--border)] bg-[var(--surface-muted)] px-5 py-5"
                        >
